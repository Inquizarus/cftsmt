...
func TestThatDefaultProviderCreatesValidRemoveStatement(t *testing.T) {
	...
	for _, c := range cases {
		assert.Equal(t, c.expected, fp.Remove(c.resource))
	}
}

func TestThatDefaultProviderCreatesValidImportStatement(t *testing.T) {
	fp := formatting.DefaultProvider{}
	cases := []struct {
		expected string
		resource terraform.StateResource
	...
	for _, c := range cases {
		assert.Equal(t, c.expected, fp.Import(c.resource))
	}
}
